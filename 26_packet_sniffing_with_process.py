...
class Packet:
    def __init__(self, pid, process, source_port, ip, destination_port, count):
        self.pid = pid
...
def print_results(all_packets):
    table = PrettyTable()
    table.field_names = ["PID", "Process", "Source port", "Destination address", "Destination port",  "# Packets captured"]

    for item in sort_result(all_packets):
        ip = item[0]
        packet = item[1]
        table.add_row([packet.pid, packet.process, packet.source_port, ip, packet.destination_port, packet.count])
